...
class SqlPylusConnection(object):
    ...
    def execute(self, sql, timeout=None):
        ...
        if self._sql_plus_process.returncode != 0:
            ...
            if re.findall("(ORA|SP2|TNS)-(\d+)", whole_text):
                raise SqlPylusException("Execute Sql Error {}".format(
    ...
    def _connect(self):
        ...
        try:
            ...
            raise SqlPylusException("connect failed. base error[{}]".format(e))
...
class SqlPylus(object):
    ...
    def connect(self, user, password, dsn=None, oracle_sid=None, is_sysdba=False):
        ...
        self.environ["NLS_LANG"] = 'American_America.{}'.format(self.client_encoding)
...
class SqlplusHtmlResultParser(HTMLParser):
    """
    ...
    def handle_endtag(self, tag):
        ...
        self._last_tag = "/{}".format(tag.upper())
...
if __name__ == '__main__':
    ...
    print(conn)
    for row in conn.execute('select * from v$version'):
        print(row)
